 ...
 * 
 */
public class CreateReservationResource extends ServerResource {
	...
	public Representation create(String data) {
		...
		try {
			...
			while (
				!resvStatus.equals(OSCARSClient.STATUS_RESERVED) &&
				!resvStatus.equals(OSCARSClient.STATUS_ACTIVE) &&
				!resvStatus.equals(OSCARSClient.STATUS_FAILED) &&
				!resvStatus.equals(OSCARSClient.STATUS_CANCELLED)) {
				// send query
				QueryResContent queryRequest = new QueryResContent();
				queryRequest.setGlobalReservationId(gri);
				QueryResReply queryReponse = client
						.queryReservation(queryRequest);

				// check status
				resvStatus = queryReponse.getReservationDetails().getStatus();
				if (resvStatus.equals("FAILED")) {
					System.err.println("Circuit reservation failed");
					return answer("Circuit reservation failed");
				}
